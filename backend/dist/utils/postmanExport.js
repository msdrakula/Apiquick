...
exports.collectionToPostman = collectionToPostman;
exports.loadCollectionExport = loadCollectionExport;
...
function collectionToPostman(col, requests) {
    ...
    for (const folder of folders) {
        ...
        if (!folder)
            ...
            items.push({ name: folder, item: reqs });
    }
    return {
        info: {
...
function loadCollectionExport(col) {
    const requests = (0, db_1.all)('SELECT * FROM requests WHERE collection_id = ? ORDER BY folder, id', [col.id]);
    return collectionToPostman(col, requests);
}
//# sourceMappingURL=postmanExport.js.map