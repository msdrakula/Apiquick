...
router.post('/bru', (req, res) => {
    try {
        ...
        const bodyContent = parsed.body ? Object.values(parsed.body)[0] : '';
    ...
    `, [
        ...
        const id = (0, db_1.lastId)('requests');
...
router.get('/bru/:requestId', (req, res) => {
    try {
        const row = (0, db_1.get)('SELECT * FROM requests WHERE id = ?', [req.params.requestId]);
        if (!row)
