...
function randDigits(len: number) { return Array.from({length: len}, () => Math.floor(Math.random() * 10)).toString().replace(/,/g, ''); }

export function generateBuiltinVariables(): Record<string, string> {
  const d = new Date();
  return {
